...
> Это TODO-лист: цели заводятся вручную в UI Метрики
> (Настройки → Цели). После создания — поставить галочку.
> Счётчик — 109130279. Тип цели — «JavaScript-событие».
> Все имена событий ниже подтянуты из кода компонентов через
> `track()` / `trackOnce()` в `src/utils/track.ts`.
