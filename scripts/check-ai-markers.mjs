 ...
 *
 * `\b` в JavaScript — это граница между [A-Za-z0-9_] и всем остальным, поэтому
 * вокруг русских слов он не срабатывает вообще: `/\bявляется\b/.test('это
 * является примером')` возвращает false. Из-за этого одиннадцать маркеров ниже
 * («является», «данный», «осуществляется» и прочие первые номера редполитики)
 * не ловились ни разу, а итоговый скор был занижен на их вес.
 *
 * То же решение уже применено в scripts/content/lib/article-rules.mjs (wordRe)
 * — здесь оно просто не было продублировано.
 */
const ru = (body, flags = 'gi') =>
  new RegExp(`(?<![а-яёА-ЯЁa-zA-Z])(?:${body})(?![а-яёА-ЯЁa-zA-Z])`, flags);

/**
 * Сколько вхождений одного маркера идёт в скор. Остальные показываем автору,
 * но не считаем: повтор одного слова — одна проблема, а не N проблем.
 */
const HITS_COUNTED_PER_MARKER = 3;
...
const MARKERS = [
  ...
  [ru('данная'),                              1, '→ эта'],
  [ru('данное'),                              1, '→ это'],
  // «Данные» ловим только в роли местоимения («данные товары» = «эти товары»).
  // Существительное отсекаем по двум признакам: слева согласованное
  // прилагательное («персональные данные», «фискальные данные») либо справа
  // предлог/глагол («данные о владельце», «данные передаются»). Без этого
  // статья про 152-ФЗ набирала 7 баллов из 10 на одном этом слове.
  // Границы слова здесь только через lookaround: `\b` в JS считает границей
  // стык любых двух кириллических букв с пробелом, поэтому на русском тексте
  // молча не срабатывает — на этом уже горели все маркеры выше.
  [new RegExp(
    '(?<![а-яё]+(?:ые|ие|ых|их|ыми|ими)\\s)'
    + '(?<![а-яёА-ЯЁa-zA-Z])данные(?![а-яёА-ЯЁa-zA-Z])'
    + '(?!\\s+(?:из|об|о|по|за|в|от|для|с|при|на|к)(?![а-яёА-ЯЁa-zA-Z]))'
    + '(?!\\s+[а-яё]+(?:ется|ются|лись|ся|ли|ло|ла)(?![а-яёА-ЯЁa-zA-Z]))',
    'gi'), 1, '→ эти (если не «данные» как существительное)'],
...
function analyzeRegex(text, filename) {
  ...
  for (const [re, weight, fix] of MARKERS) {
    ...
    while ((m = re.exec(body)) !== null) {
      ...
      seen++;
      // Одно и то же слово, повторённое двадцать раз, — это одна стилевая
      // проблема, а не двадцать. Без потолка любой частотный маркер в
      // тематической статье в одиночку перебивал порог блокировки, и автору
      // приходилось вычищать слово ради шлюза, а не ради читателя.
      const counted = seen <= HITS_COUNTED_PER_MARKER;
      hits.push({ line: lineNo, phrase: m[0], fix, weight, counted });
      if (counted) totalWeight += weight;
    }
  }
...
for (const { label, regex, llm, finalScore } of results) {
  ...
  if (regex.hits.length > 0) {
    ...
    for (const h of regex.hits.sort((a, b) => a.line - b.line)) {
      const mark = h.counted ? '' : ' (сверх потолка, в скор не идёт)';
      console.log(String(h.line).padEnd(6) + h.phrase.padEnd(45) + h.fix + mark);
    }
  }
