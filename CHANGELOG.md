...
## 2026-08-24 — Автономная публикация статей

Сайт молчал с 06.07.2026: `auto-publish.yml` умеет только выпускать готовые
черновики, а писал их человек через `/create-article`. Очередь опустела,
workflow полтора месяца отрабатывал вхолостую, сигнализации на этот случай
не было.

### Что появилось

- **`content-autopilot.yml`** — раз в месяц пишет норму статей (по умолчанию
  22, по будним дням) и прогоняет hero-картинки **одним пакетом** на всю
  норму. По понедельникам дозаправляет очередь, если запас просел ниже
  10 дней.
- **`scripts/content/generate-drafts.mjs`** — конвейер статьи: research с
  веб-поиском → написание → шлюз качества с починкой → онлайн-фактчек НПА и
  сумм → соцчерновики. Всё на существующем `OPENROUTER_API_KEY`, новых
  секретов не нужно.
- **`scripts/content/qa-gate.mjs`** — шлюз перед снятием `draft`: редполитика,
  маркер фактчека, подтверждение номеров НПА, скор AI-маркеров. Не прошедший
  черновик не блокирует очередь, причина пишется в `.claude/blocked/<slug>`.
- **`scripts/content/lib/article-rules.mjs`** — редполитика в исполняемом
  виде: одни и те же правила у генератора и у шлюза.
- **`scripts/content/queue-status.mjs`** — запас публикаций одной командой.
- **`scripts/content/build-guard.mjs`** — битая свежая статья уезжает в
  карантин, сборка пересобирается; одна статья не съедает месячную норму.
- **`docs/autopilot.md`** — описание контура, настройки, как вмешаться и как
  остановить.

### Что изменилось

- `release-next-draft.mjs` выпускает черновик **по наступившей `pubDate`** и
  только после шлюза качества. Статьи выходят по одной в будний день, а не
  четырьмя пачками в сутки.
- `auto-publish.yml`: пустая очередь **роняет прогон** — GitHub присылает
  письмо. Hero-картинка генерится только если её нет (обычно она уже
  нарисована пакетом).
- `generate-hero-images.mjs`: флаг `INCLUDE_DRAFTS=1` — прогон по черновикам.
- `health-check.mjs`: черновики больше не считаются тревогой, тревога —
  пустая очередь. Красный при запасе публикаций < 3 дней.

### Замечания

- Слова-паразиты в проверках раньше не ловились: `\b` в JS не работает как
  граница слова для кириллицы. Заменено на явные lookaround'ы.
- Запас тем в контент-плане на момент запуска — 138 штук, около 6 месяцев
  публикаций.
