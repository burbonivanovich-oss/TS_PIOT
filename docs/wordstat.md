...
Поля:
- `shows` — последний полный месяц истории. Используется как «текущая частота».
- `trend` — `up`/`down`/`stable`/`unknown`. Считается как отношение среднего за
  последние 3 мес к среднему за предыдущие 3.
...
- `related` — топ-30 похожих запросов с их частотой. Источник идей для новых
  статей (вход для `/find-topics`). Заполняется только для верхних 50 P1-ключей.
...
| `frontmatter.tags` опубликованной статьи | 2 | Обновляем не чаще 1 раза в `4 × FRESH_DAYS` |
...
- `POST https://api.wordstat.yandex.net/v1/dynamics` — частота по месяцам за 12
  мес. **1 квота на ключ.**
- `POST https://api.wordstat.yandex.net/v1/topRequests` — похожие запросы.
  **1 квота на ключ.** Используется выборочно (TOP_REQUESTS_LIMIT).

Лимиты по умолчанию для OAuth-токена:
- 1000 запросов в сутки.
- 10 запросов в секунду.

`MAX_QUOTA=500` оставляет половину суточного лимита на ручные запуски и
расширения.
