...
**FLUX для превью** — быстрый, дешёвый (`flux-1-schnell` бесплатен), хорошо
делает плоские editorial-иллюстрации в едином стиле. Превью — это «обложка
журнала», важна согласованность стиля между статьями, а не фотореализм.

**FLUX для hero** — тот же провайдер, единый ключ (`OPENROUTER_API_KEY`).
Для hero используется более детальный фотореалистичный промпт с тёмным фоном,
подходящим для наложения белой типографики. При необходимости модель можно
сменить через `HERO_MODEL=black-forest-labs/flux-1-1-pro`.
...
|---|---|---|---|
| Generate Article Images | `.github/workflows/generate-article-images.yml` | push `*.md` в `main` / `workflow_dispatch` | Preview (Riverflow v2 Fast) + Hero (FLUX.2 Klein) за один прогон |
| Generate Hero Images | `.github/workflows/generate-hero-images.yml` | `workflow_dispatch` | Только hero-изображения через FLUX (OpenRouter) |
...
Все три workflow требуют `workflow_dispatch` из UI и работают только от `main`-ветки.
...
| Поле | Значение |
|---|---|
| `slug` пусто | Генерировать для всех статей без `previewImage` / `heroImage` (только `draft: false`) |
...
### Автозапуск при пуше статей

Workflow **Generate Article Images** запускается автоматически при пуше любого
`.md`-файла в `src/content/blog/` на ветку `main`. Это значит: как только статья
переходит из `draft: true` в `draft: false` и попадает в `main` — изображения
генерируются без ручного запуска.
