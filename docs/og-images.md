...
|---|---|---|---|---|
| **Preview** | `previewImage` | `public/images/preview/{slug}.jpg` | Nano Banana (OpenRouter) | На каждую статью |
| **Hero** | `heroImage` | `public/images/hero/{slug}.jpg` | Nano Banana (OpenRouter) | На каждую статью |
| **OG фон** | — | `public/og-backgrounds/{cat}.jpg` | Nano Banana или любая (выбор) | При смене стиля |
...
| Группа | Модели | Цена |
|---|---|---|
| Google (OpenRouter) — **default** | `google/gemini-3.1-flash-image-preview` (Nano Banana) | низкая |
| Black Forest Labs (OpenRouter) | `flux-1-schnell` (бесплатно), `flux-1-1-pro`, `flux-2-max` | $0–$0.06/img |
...
| `generate-og-backgrounds-openrouter.mjs` | AI фоны через OpenRouter (Nano Banana / FLUX) |
