...
## Как выбирается промпт (`buildPrompt`)

Порядок приоритетов в `scripts/generate-hero-images.mjs`:

1. **`SLUG_PROMPTS[slug]`** — точный ручной промпт под конкретную статью. Высший приоритет.
2. **`SUBJECT_MAP` (предметная страховка)** — если точного промпта нет, по ключевым
   словам в slug определяется **реальный предмет статьи** (обувь → коробки с обувью,
   табак → пачки сигарет, снеки/соусы/специи → снеки и приправы) и подставляется
   в один из вариативных `SCENE_TEMPLATES`.
3. **`CAT_POOLS[category]`** — последний фолбэк: пул сцен по категории.

### Зачем добавлен `SUBJECT_MAP`

Раньше у статьи без точного промпта картинка бралась **случайно** из пула категории
(`pool[slugHash(slug) % pool.length]`). Это давало грубые промахи по теме:

- «Маркировка обуви» получала *молочные бутылки* (хэш выбрал dairy-сцену из пула `markirovka`).
- «Маркировка снеков/соусов/специй» получала макароны (старый slug-промпт говорил «cereals, pasta»).

`SUBJECT_MAP` гарантирует, что кадр всегда строится **вокруг предмета статьи**, а не
случайной сцены. Ключи упорядочены от узких к общим; матчинг — по `slug.includes(key)`.
При добавлении новой товарной группы — добавляйте ключ→предмет в `SUBJECT_MAP`
(а не только в пул категории).

### `SCENE_TEMPLATES` — разнообразие

Один и тот же предмет разворачивается в одну из **14 композиций**, выбор — по
`slugHash(slug)`. Сцены намеренно разнесены по ракурсу, свету, окружению и времени
суток: retail close-up, flat-lay, склад/паллета, момент сканирования на кассе,
macro-деталь, интерьер магазина, low-angle hero, top-down на тёмном сланце,
golden-hour у деревянного прилавка, студийный seamless, вид из-за прилавка,
подсобка, плотный overhead-кроп, отражающая тёмная столешница. Так смежные статьи
кластера не выглядят одинаково. Чтобы добавить ещё разнообразия — расширяйте
массив новыми сценами, не трогая предметы из `SUBJECT_MAP`.

> **Перегенерация уже опубликованных hero**: если у статьи картинка не по теме,
> правьте `SLUG_PROMPTS`/`SUBJECT_MAP` и запускайте генерацию с `FORCE=1 SLUG=<slug>`.
> Кривые на момент правки: `2026-05-27-markirovka-obuvi-2026`,
> `2026-05-03-markirovka-bakalei-2026` — перегенерировать с `FORCE=1`.
