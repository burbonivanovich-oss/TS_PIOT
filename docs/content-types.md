# Как добавить контент

Пошаговые инструкции для каждого типа контента в проекте.

## Статья (коллекция `blog`)

1. Создайте файл `src/content/blog/YYYY-MM-DD-slug.md`. Дата в имени —
   формальность для сортировки в файловом менеджере; на URL влияет только
   `slug` (всё имя файла без расширения = id записи).
2. Frontmatter:

   ```yaml
   ---
   title: "Заголовок"
   description: "Описание для SERP, до 160 символов."
   pubDate: "YYYY-MM-DD"
   updatedDate: "YYYY-MM-DD"   # необязательно — поднимает «свежесть» в листингах
   tags:
     - тег1
     - тег2
   categories:
     - ts-piot   # или markirovka, zakonodatelstvo, kkt, egais
   draft: false
   cpa: "chestny-znak"         # необязательно — id баннера из src/data/cpa-banners.ts
   seo:
     keywords:
       - целевой ключ
   ---
   ```

3. **`heroImage` указывать не обязательно.** Без него `BlogPost.astro`
   подставляет в `og:image` автогенерируемую `/og/<slug>.png` (см. [og-images.md](og-images.md)).
4. Теги указываются на русском, с пробелами и заглавными — slug формируется
   автоматически (`tagSlug` в `src/utils/tags.ts`).
5. После публикации страница появится на `/blog/<slug>/`, попадёт в архив
   `/blog/`, в категории `/category/<категория>/` и в каждый тег `/tag/<slug>/`.

### CPA-баннер

Каждая статья показывает партнёрский/редакционный баннер в контентной колонке.
Баннер выбирается так:

1. Если в frontmatter есть `cpa: "<id>"` — используется этот баннер.
2. Иначе — баннер по умолчанию для категории статьи (из `CATEGORY_DEFAULT_CPA`
   в `src/data/cpa-banners.ts`).
3. Запасной вариант — `'default-ts-piot'`.

Доступные id баннеров:

| id | Назначение |
|---|---|
| `chestny-znak` | Регистрация в «Честном знаке» |
| `default-ts-piot` | Подключение ТС ПИоТ до 01.07.2026 |
| `default-markirovka` | Проверка категорий маркировки |
| `default-zakonodatelstvo` | Калькулятор штрафов |

Чтобы добавить новый баннер — добавьте запись в `CPA_BANNERS` в `src/data/cpa-banners.ts`.

### FAQ в статье

FAQ (аккордеон с FAQPage JSON-LD) — **необязательный** элемент. Используйте его,
когда статья описывает процедуру или регулирование, по которым у читателей
возникают конкретные частые вопросы.

**Когда добавлять FAQ:**

- Инструкционный материал («как подключить», «что делать если...»)
- Обзор требований и норм с чёткими границами (кому, когда, как)
- Материал с высокой вероятностью вопросов о сроках, штрафах, исключениях
- Статья целится на Featured Snippet или «Люди также спрашивают»

**Когда FAQ не нужен:**

- Новостная заметка или объявление об изменении
- Аналитический обзор/мнение без четкого Q&A потенциала
- Статья и так полностью структурирована по вопросам (H2 = вопросы)
- Материал менее 800 слов — FAQ раздует его неоправданно

**Использование:**

```mdx
import FAQ from '../../components/FAQ.astro';

<FAQ
  items={[
    { q: 'Нужно ли ИП подключать ТС ПИоТ?', a: 'Да, ИП с онлайн-кассой...' },
    { q: 'Что будет, если не подключить до 01.07.2026?', a: 'Касса заблокирует продажи...' },
  ]}
/>
```

Компонент автоматически добавляет `FAQPage` JSON-LD — его дополнительно прописывать не нужно.
Размещайте FAQ в конце статьи, перед заключительным абзацем.

### Акцентные врезки (Callout)

Два способа использования:

**1. В MDX-файлах — через компонент (с авто-лейблом):**

```mdx
import Callout from '../../components/Callout.astro';

{/* Семантические типы — лейбл и цвет проставляются автоматически */}
<Callout type="penalty">ИП — 5 000–15 000 ₽, юрлицо — 50 000–300 000 ₽ (ст. 15.12 КоАП РФ).</Callout>
<Callout type="date">ТС ПИоТ обязателен с 1 июля 2026 года.</Callout>
<Callout type="warning">Без ТС ПИоТ касса не пробьёт чек на маркированный товар.</Callout>
<Callout type="tip">Зарегистрируйтесь в «Честном знаке», обновите прошивку кассы, заключите договор с ОФД.</Callout>

{/* Декоративные варианты — без лейбла, для цитат */}
<Callout variant="dark">Чёрный фон, лаймовый акцент.</Callout>
<Callout variant="light">Песочный фон, розовый акцент.</Callout>
```

| type | Лейбл | Цвет | Когда |
|---|---|---|---|
| `penalty` | Штраф | Красный | После секции со штрафами |
| `date` | Важная дата | Синий | После лида или H2 с дедлайном |
| `warning` | Важно | Жёлтый | Критическое требование |
| `tip` | Что сделать | Зелёный | После пошагового раздела |

**2. В обычных .md-файлах — через HTML-div (CSS `::before` подставляет лейбл):**

```html
<div class="callout callout--penalty">
ИП — 5 000–15 000 ₽, юрлицо — 50 000–300 000 ₽ (ст. 15.12 КоАП РФ).
</div>
```

Правило: 1–2 врезки на статью, контент — только из текста статьи.

### Связи между постами

- **«Читайте также»** генерируется автоматически в `BlogPost.astro`. Скоринг:
  совпадение тегов ×2, совпадение категорий ×1. Берётся до 4 совпадающих постов.
  Макет выбирается по количеству:
  - 2 поста → два равных столбца (A2)
  - 3 поста → 50/50 вариант A3 или B (детерминировано по slug статьи)
  - 4+ поста → одна большая карточка + три компактных (C)
- **Внутренние ссылки** в теле статьи — относительные (`/blog/...`,
  `/category/...`, `/slovar/#тс-пиот`).

## Термин (коллекция `glossary`)

Создайте `src/content/glossary/<slug>.md`. Slug — латинский, дефис-разделённый
(используется в имени файла, не в URL — на URL идёт `termSlug` от `term`).

```yaml
---
term: "ТС ПИоТ"
aliases:
  - "техническое средство получения информации о товаре"
summary: "Краткое определение для DefinedTerm JSON-LD и SERP."
category: "ts-piot"   # необязательно: ts-piot, markirovka, zakonodatelstvo, kkt, egais
relatedLinks:
  - label: "Категория: ТС ПИоТ"
    url: "/category/ts-piot/"
---

Тело — развёрнутое определение в Markdown. 1–2 абзаца.
```

После сборки термин появится на `/slovar/` под буквой первой Cyrillic-буквы
`term`. Якорь — `#<termSlug(term)>`. Сортировка алфавитная.

## Pillar-страница категории (коллекция `pillars`)

`src/content/pillars/<slug>.md`. Имя файла **должно совпадать** со slug
категории из `CATEGORIES` в `src/consts.ts`.

```yaml
---
title: "Заголовок pillar (если отличается от fullTitle категории)"
summary: "1–2 предложения для description в meta и hero-блока."
keyDates:
  - date: "01.07.2026"
    event: "Описание события."
faq:
  - question: "Вопрос?"
    answer: "Ответ."
---

Тело pillar в Markdown — 3–4 абзаца контекста: что это, кому, как и почему.
```

Что делает шаблон `category/[category].astro`:

1. Хлебные крошки и H1 (из `meta.fullTitle`).
2. Лид (`pillar.data.summary` или `meta.description`).
3. Тело pillar (`Content` из render).
4. Блок «Ключевые сроки» (если есть `keyDates`).
5. FAQ-аккордеон (если есть `faq`) + `FAQPage` JSON-LD.
6. Список постов категории (отсортированных по `pubDate desc`).

Если файла pillar нет — шаблон работает в fallback-режиме: показывает hero
из CATEGORIES и сразу список постов.

## Сценарий калькулятора штрафов

Файл `src/data/penalties.ts`, массив `SCENARIOS`. Это единственный источник:
страница `/kalkulyator-shtrafov/` рендерит form, список и FAQ из этого массива.

```ts
{
  id: 'unique-id',
  label: 'Что произошло (для select)',
  short: 'Краткое описание (для блока "Все учтённые сценарии")',
  article: 'ст. X.Y КоАП РФ',
  calc: {
    type: 'fixed',
    ip:    { min: 5000, max: 15000 },
    legal: { min: 50000, max: 300000 },
  },
  // или для процентного штрафа:
  // calc: {
  //   type: 'percent',
  //   ip:    { minPct: 25, maxPct: 50, floor: 10000 },
  //   legal: { minPct: 75, maxPct: 100, floor: 30000 },
  // },
  confiscation: true,        // показывает плашку «+ конфискация»
  note: 'Дополнительное пояснение под результатом.',
}
```

При обновлении сумм:

1. Сверьте с действующей редакцией КоАП на pravo.gov.ru.
2. Проверьте, не противоречите ли уже опубликованным постам:
   `grep -nE '14\.5|15\.12' src/content/blog/`. Если расходитесь —
   обновите соответствующие посты в одном коммите с калькулятором.

## Категория (новая)

Если действительно нужна **шестая** категория (а не сателлит существующей — уже есть 5: ts-piot, markirovka, zakonodatelstvo, kkt, egais):

1. Добавьте запись в `CATEGORIES` (`src/consts.ts`):
   ```ts
   'new-slug': {
     title: 'Краткое имя',
     fullTitle: 'Полное имя для H1',
     description: 'Описание ...',
   },
   ```
2. Расширьте enum в `src/content.config.ts` для коллекций `pillars`
   (поле `category` пока не используется напрямую, но в `glossary`
   есть `category`-enum) и `glossary`.
3. Создайте `src/content/pillars/new-slug.md` с базовым intro/keyDates/FAQ.
4. Добавьте дефолтный CPA-баннер в `src/data/cpa-banners.ts` и в `CATEGORY_DEFAULT_CPA`.
5. По желанию — добавьте пункт в `NAV_LINKS`.
6. После сборки появится `/category/new-slug/`.

## Социальные черновики (`src/content/wiki/social/`)

Для каждой статьи создаётся файл `src/content/wiki/social/<slug>.md`
с черновиками для четырёх платформ. Генерируется автоматически через
`social-media-manager` в рамках пайплайна `/new-post`.

### Frontmatter

```yaml
---
title: "Название статьи"
slug: "YYYY-MM-DD-slug"
articleUrl: "/blog/YYYY-MM-DD-slug/"
status: draft        # draft | ready | posted
createdDate: "YYYY-MM-DD"
---
```

### Структура

```markdown
## Telegram
[3–5 абзацев, конкретика, без эмодзи]
etiketka-media.ru/blog/<slug>/

---

## VK
[Развёрнутее Telegram, списки допустимы. В конце 3–5 хэштегов]
#хэштег1 #хэштег2

---

## Дзен
**Заголовок Дзен:** [60–80 символов, отличается от H1 статьи]
[Текст 300–500 слов, журнальный стиль]

---

## Email
**Тема:** [до 50 символов]
**Прехедер:** [80–100 символов]
[150–250 слов + CTA]
[Читать статью →](/blog/<slug>/)
```

### Правила платформ

- **Telegram**: сухо и по делу. Первые 2 строки цепляют без «дорогой читатель».
  Никаких эмодзи. Последняя строка — `etiketka-media.ru/blog/<slug>/`.
- **VK**: чуть теплее, допустим риторический вопрос. Хэштеги в конце.
- **Дзен**: заголовок-обещание (нельзя дублировать H1), конкретика в первом абзаце.
- **Email**: тема ≤ 50 символов (выгода или вопрос), один CTA.

Статус переключается вручную: `draft → ready → posted`.

## Чего лучше не делать

- Не создавайте `<h1>` внутри тела статьи — он один на страницу, в шаблоне.
- Не указывайте абсолютные URL во внутренних ссылках — только относительные.
- Не дублируйте посты под близкие запросы — обновляйте существующие
  (`updatedDate` в frontmatter).
- Не правьте маршруты в `astro.config.mjs` — оставьте `trailingSlash: 'always'`.
- Не добавляйте FAQ ради FAQ — только там, где у читателей реально возникают вопросы.
