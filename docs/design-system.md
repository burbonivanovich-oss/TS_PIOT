...
| Секция | Что внутри |
|---|---|
| Hero | Eyebrow + h1 + lead + 2 CTA (Все материалы / Подобрать сервис) |
| Свежее | Featured pillar слева (16:10 фото + tag + h3 + лид + «Читать →»), 5 цветных category-cards справа (LAB3 #13 в малом размере): ТС ПИоТ чёрная, Маркировка pink, Законы blue, Кассы purple, **ЕГАИС lime** |
| Материалы недели | 6 карточек статей 3×2: плейсхолдер 16:9 (radial-градиент по категории + диагональные полосы) или реальное `previewImage`/`heroImage`, tag, h3, дата mono |
| Инструменты | 5 интерактивов: Симулятор ТС ПИоТ (pink, span:3), Тест готовности (lime, span:3), Кофейня (blue), ИП→ООО (white), Законы 2026 (dark) |

Заголовки секций без префикса-номера (`01/02/…`). Пробовали с
нумерацией, ушли — это лента, не журнал-расклад, нумерация мешала.
...
- «Серия материалов», «По категориям» (большие плашки подряд), «CTA band»,
  «Mini-newsletter», «По формату бизнеса» — рассматривались в прототипе,
  не пошли в продакшн.
...
| Секция | Что |
|---|---|
| Hero | Без правой колонки с анонсами. Eyebrow + h1 + лид + 2 CTA (Все материалы / Подобрать сервис) |
| Свежее | Последний pillar слева (16:10 фото) + 5 категорий справа цветными мини-плашками |
| Материалы недели | 6 карточек 3×2 с плейсхолдерами по категории (radial-градиент + диагональные полосы) или с реальными `previewImage` |
| Инструменты | 5 интерактивов (Симулятор, Тест, 2 сценария, Трекер 2026) |

Заголовки секций без нумерации `01/02/…`. Блок «По формату бизнеса»
(подборки по типу читателя) рассматривался в проторипе, в продакшн
не пошёл — добавлял повторную навигацию поверх существующих категорий
и инструментов.
