...
**Статус на 04.09.2026: секрет не заведён**, поэтому шаг «Fetch Webmaster»
падает каждый прогон, а `src/data/analytics/webmaster.json` остаётся
заглушкой с нулями. Токен получается за пару минут по разделу «Яндекс OAuth:
общие шаги» выше — это тот же implicit flow, что и для Метрики, только
приложению нужен scope `webmaster:hostinfo`. Отдельное приложение заводить
не обязательно: можно добавить этот scope существующему приложению Метрики
и перевыпустить токен, тогда одним токеном закрываются оба секрета.
...
**Главная ловушка.** Для приложений в статусе **Testing** в Google Cloud
Console refresh_token протухает через **7 дней** — по календарю, независимо
от того, пользуетесь вы им или нет. Раньше в этом разделе было написано, что
регулярный рефреш решает проблему; это неверно, и именно так GSC и отвалился:
прогон 04.09.2026 получил

```
OAuth refresh 400: {"error": "invalid_grant", "error_description": "Bad Request"}
```

Лечится одним из двух способов:

1. **Publish в Google Cloud** (Audience → Publish app). Для scope
   `webmasters.readonly` и внутреннего использования review обычно не нужен —
   после публикации refresh_token живёт, пока его не отозвали. Это правильный
   путь, делать один раз.
2. Перевыпускать токен вручную раз в неделю по `docs/analytics.md`. Годится
   только как временная мера.

Проверить, что токен жив, можно не дожидаясь понедельника: Actions →
**Аналитика — обновление данных** → Run workflow. Шаг «Состояние источников»
в конце прогона печатает таблицу по всем трём источникам и красит прогон,
если какой-то из них не отдал свежие данные.
