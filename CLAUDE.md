...
| **`docs/autopilot.md`** | Автономная публикация: кто пишет статьи, шлюзы качества, расписание, как вмешаться и как остановить |
...
| `docs/audit-2026-09-03.md` | Аудит 03.09.2026: код, автоматизация, контент, UX; приоритеты и план на две недели |
...
По умолчанию сайт публикуется **сам**: Routine на claude.ai/code раз в неделю
пишет черновики агентами проекта (research → writer → seo → social) с
расставленными `pubDate`, картинки досыпает `content-images-batch.yml`, а
`auto-publish.yml` выпускает по одному черновику в день после шлюза качества.
Полное описание контура, промпт Routine и способы вмешаться —
**`docs/autopilot.md`**.

**OpenRouter в проекте — только для изображений.** Текст пишет Claude по
подписке; ни один шаг работы с текстом не должен ходить в OpenRouter.

Быстрая диагностика: `node scripts/content/queue-status.mjs` — сколько
черновиков в очереди и на сколько дней хватит запаса.
Задание на следующий прогон: `node scripts/content/plan-run.mjs`.

Ручной путь ниже нужен, когда тема резонансная и цена ошибки высока:
автопилот на такие темы лучше не пускать.

## Workflow создания статьи вручную
