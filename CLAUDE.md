...
| **`docs/editorial-cycle.md`** | Автоматический цикл с редактором-человеком: рутины A/B/C, машина состояний, интерфейс согласования в GitHub Issues |
...
`/cycle-plan` → `/cycle-listen` → `/cycle-batch`. План согласуется с
редактором-человеком в GitHub Issue, статьи пишутся батчами по 3 штуки,
новый батч не стартует, пока не влит предыдущий.
